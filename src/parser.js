...
class Parser {
    ...
    parseAssign(type) {
        ...
        current.name = this.prev;
        this.eat("=", "assign");
        this.eat(null, "identifier", "string", "number");
        current.value = this.prev;
        current.data = this.prev.type;
        this.eat(";", "semi");
        return current;
    }
    parseFunCall() {
    ...
    parseBlock() {
        ...
        while (true) {
            if (this.index >= this.tokens.length - 1) {
                console.log(`Unexpected End of input was expecting a closing body cahracture "]" ---> ${this.lexer.file_name}[${this.cur.line}:${this.cur.col}]`);
                process.exit(1);
            }
            if (this.cur.value == "]")
                break;
    ...
    parseArgs() {
        ...
        while (true) {
            if (this.index >= this.tokens.length - 1) {
                console.log(`\x1b[31mError\x1b[0m: Unexpected End of FIle, was expectinga ")" characture ---> \x1b[4m${this.lexer.file_name}\x1b[0m[${this.cur.line}:${this.cur.col}]`);
                process.exit(1);
            }
            if (this.cur.value == ")")
                break;
    ...
    parse() {
        while (true) {
            if (this.cur.type == "EOF" || this.index >= this.tokens.length)
                break;
