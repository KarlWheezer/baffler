...
export class Parser {
    tokens: token[]; lexer: Lexer;
    index: number; statements: any[];
    cur: token; prev: token;
    next: token;
    constructor(lexer: Lexer) {
        this.index = 0; this.lexer = lexer;
    ...
    eat(value: string | null = null, ...type: any[]) {

        if (this.index >= this.tokens.length) {
            let i = 0; let buffer = ""; while (i < this.prev.col - 1) { i ++; buffer += " " }
            buffer += "^"; i = 0; while (i < this.prev.len - 1) { i ++; buffer += "~" }
            let EOFmsg = [
            ...
            if (value !== null) EOFmsg[EOFmsg.length - 1] += ` with value '${value}'.`
            console.log(EOFmsg.join("\n")); process.exit(1);
        }

        if (value === null) { for (let i=0; i < type.length; i ++) { if (this.cur.type == type[i]) { this.advance(); return true; } } }
        if (value !== null) { for (let i=0; i < type.length; i ++) { if (this.cur.type == type[i] && this.cur.value == value) { this.advance(); return true; } } }

        let i = 0; let buffer = ""; while (i < this.cur.col - 1) { i ++; buffer += " " }
        buffer += "^"; i = 0; while (i < this.cur.len - 1) { i ++; buffer += "~" }
