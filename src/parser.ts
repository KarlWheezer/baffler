...
export class Parser {
    lexer: Lexer; index: number;
    tokens: token[]; cur: token;
    next: token; prev: token;
    statements: object[];
    constructor(lexer: Lexer) {
        this.lexer = lexer; this.index = 0;
    ...
    eatLiteral() {
        ...
        for (var i = 0; i < lits.length; i ++) {
            if (this.cur.type == lits[i]) { this.advance(); return true }
        }
        let k=0; let bufr = ""; while (k < this.cur.col - 1) { k ++; bufr += " " }; bufr += "^"
        k = 0; while (k < this.cur.len - 1) { bufr += "~"; k ++ }
        let msg = [
    ...
    eat(type: string = "identifier", value: string | undefined = undefined) {
        if (value === undefined) { if (this.cur.type === type) { this.advance(); return true; } }
        else if (value !== undefined) { if (this.cur.type === type && this.cur.value === value) {this.advance(); return true; } }
    ...
    parse() {
        while (this.index < this.tokens.length) {
            if (this.cur.type == "keyword" && this.cur.value == "set") {
                ...
                this.eat("keyword", "set"); this.eat("identifier"); cs.name = this.prev;
                this.eat("colon", ":"); this.eat("data type"); cs.data_type = this.prev;
                this.eat("assign", "="); this.eatLiteral();    cs.value = this.prev;
                this.eat("semi colon", ";"); 
                
                if (cs.data_type.value !== cs.value.type) this.typeError(cs.value.type, cs.data_type.value);
                ...
                this.eat("keyword", "var"); this.eat("identifier"); cs.name = this.prev;
                this.eat("colon", ":"); this.eat("data type"); cs.data_type = this.prev;
                this.eat("assign", "="); this.eatLiteral();        cs.value = this.prev;
                this.eat("semi colon", ";"); 
        ...
        writeFileSync(this.lexer.file_name + ".json", JSON.stringify(this.statements, null, 4))
    }
}